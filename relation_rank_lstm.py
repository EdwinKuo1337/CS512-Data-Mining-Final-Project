...
class MyModel(tf.keras.Model):
    def __init__(self):
        super().__init__()
        self.rel_weight = tf.keras.layers.Dense(units=1, activation=leaky_relu)
        self.head_weight = tf.keras.layers.Dense(units=1, activation=leaky_relu)
        self.tail_weight = tf.keras.layers.Dense(units=1, activation=leaky_relu)
        self.outputs_concated_flat = tf.keras.layers.Dense(units=self.parameters['unit'], activation=leaky_relu,kernel_initializer=tf.glorot_uniform_initializer())
        # One hidden layer
        self.prediction = tf.keras.layers.Dense(units=1, activation=leaky_relu, name='reg_fc',kernel_initializer=tf.keras.initializers.GlorotUniform())

    def call(self, inputs, training=False):
        relation, rel_mask = inputs[0], inputs[1]
        feature = tf.compat.v1.placeholder(tf.float32, [self.batch_size, self.parameters['unit']])
        all_one = tf.ones([self.batch_size, 1], dtype=tf.float32)

        rel = self.rel_weight(relation)
        ...
        wei = tf.add(
            tf.add(
                tf.matmul(head, all_one, transpose_b=True),
                tf.matmul(all_one, tail, transpose_b=True)
            ), rel[:, :, -1]
        )
        weight_masked = tf.nn.softmax(tf.add(rel_mask, wei), axis=0)
        outputs_proped = tf.matmul(weight_masked, feature)
        outputs_concated = tf.concat([feature, outputs_proped], axis=1)

        pred = self.prediction(outputs_concated)

        return pred, outputs_concated
...
class ReRaLSTM:
    ...
    def train(self):
        ...
        with tf.device(device_name):
            ...
            prediction, feature_embeddings = self.mymodel.call(relation,rel_mask)

            # rel_weight = tf.keras.layers.Dense(relation, units=1,
            ...
            # else:
            #     print('sum weight')
            #     head_weight = tf.keras.layers.Dense(feature, units=1,
            #                                   activation=leaky_relu)
            #     tail_weight = tf.keras.layers.Dense(feature, units=1,
            #                                   activation=leaky_relu)
            #     weight = tf.add(
            #         tf.add(
            ...
            #     outputs_concated = tf.keras.layers.Dense(
            ...
            self.mymodel.save_weights(self.checkpoint_path)
        ...
        for i in range(self.epochs):
            checkpoint_path = './checkpoint/test_' + i + '.ckpt'
            t1 = time()
