...
class GraphNet(torch.nn.Module):
    def __init__(self):
        ...
        self.tgnn = A3TGCN2(in_channels=64,  out_channels=16, periods=12,batch_size=16, improved=True)  # node_features=2, periods=12
        # Equals single-shot prediction
        self.linear = torch.nn.Linear(1026*16, 1026)
        #self.linear2 = torch.nn.Linear(4096, 1026)
    ...
    def forward(self, x, edge_index):
        ...
        #x = x.reshape(dim0, dim1, dim2, 1) #x[16, 1026, 12, 64]
        #x_prev = torch.cat((x[0][None,:],x[:-1]),0)
        #x = torch.cat((x,x),3)
        x = torch.permute(x, (0, 1, 3, 2)) 
        h = self.tgnn(x, edge_index) # x [1, 1026, 64, 16]  returns h [b, 207, 12]
        h = F.relu(h) #[16, 1026, 16]
        h = h.reshape(dim0, -1) #[16, 1026*16]
        #h = self.linear(h)
        h = self.linear(h).reshape(dim0, -1) #[16,1026]
        return h, None
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if idx >= 11:
            emb = self.embedding[:, idx-11:idx+1]
        else:
            emb = self.embedding[:, :12]
        label = self.gt[:, idx]
...
#             edge_index[1].append(j)
    ...
    torch.save(net.state_dict(),'try.pt')
